...
class SqliteCartService {
  ...
  // In-memory fallback cho Web để dễ dàng debug
  final List<LocalCartItem> _webMemoryDb = [];

  Future<Database> get db async {
    if (kIsWeb) throw UnsupportedError('Cannot use SQLite on Web without WASM. Using memory fallback.');
  ...
  Future<void> upsertItem(LocalCartItem item) async {
    if (kIsWeb) {
      final index = _webMemoryDb.indexWhere((e) => e.productId == item.productId && e.sessionId == item.sessionId);
      if (index >= 0) {
    ...
    if (maps.isEmpty) {
      ...
      await dbClient.update(
        'local_cart',
        {'quantity': existingQty + item.quantity, 'notes': item.notes},
        where: 'product_id = ? AND session_id = ?',
        whereArgs: [item.productId, item.sessionId],
      );
    }
  ...
  Future<void> updateItemQuantity(String productId, String sessionId, int quantity) async {
    if (quantity <= 0) {
    ...
    if (kIsWeb) {
      final index = _webMemoryDb.indexWhere((e) => e.productId == productId && e.sessionId == sessionId);
      if (index >= 0) {
  ...
  Future<void> removeItem(String productId, String sessionId) async {
    if (kIsWeb) {
      _webMemoryDb.removeWhere((e) => e.productId == productId && e.sessionId == sessionId);
      return;
    }
